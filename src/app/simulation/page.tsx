...
export default function SimulationPage() {
  ...
  useEffect(() => {
    if (subjects.length > 0) {
      const results = runSimulation(studyPlan, subjects, strategy)
      setSimulationResults(results)
    }
  }, [studyPlan, subjects, strategy])
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
            ...
            <div className="p-4 bg-muted rounded-lg">
              ...
              <p className="text-sm text-muted-foreground mt-1">하루 {studyPlan.dailyStudyHours}시간</p>
            </div>
            <div className="p-4 bg-muted rounded-lg">
              <div className="flex items-center justify-center gap-2 mb-2">
        ...
        <CardContent>
          <Select value={strategy} onValueChange={(value: SimulationStrategy) => setStrategy(value)}>
            ...
            <SelectContent>
              ...
              <SelectItem value="rounds">목표 회독수 달성 우선</SelectItem>
            </SelectContent>
          </Select>
          <div className="mt-3 text-sm text-muted-foreground">
            {strategy === "equal" && "모든 과목에 동일한 시간을 할당합니다."}
            {strategy === "priority" && "과목의 우선순위에 따라 시간을 차등 분배합니다."}
            {strategy === "rounds" && "우선순위가 높은 과목부터 목표 회독수를 달성하도록 시간을 분배합니다."}
          </div>
        </CardContent>
      </Card>
